...
def get_vector_store(chunks):
    """
    Updates FAISS vector store by avoiding duplicate embeddings.
    """
    ...
    # Deduplicate by content before adding
    existing_texts = {doc.page_content for doc in vector_store.similarity_search("", k=1000)}
    new_chunks = [chunk for chunk in chunks if chunk.page_content.strip() not in existing_texts]

    if new_chunks:
        print(f"Adding {len(new_chunks)} new unique chunks...")
